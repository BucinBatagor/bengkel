...
<section class="min-h-[700px] flex flex-col items-center px-6 py-6">
    <div class="w-full max-w-screen-xl bg-white px-8 py-6 rounded-lg shadow flex-1">
        ...
        <form method="GET" action="{{ route('admin.pemesanan.index') }}" class="flex flex-wrap justify-between items-center gap-4 mb-4">
            <div class="flex items-center gap-2 relative">
                ...
                <button
                    ...
                    class="absolute right-20 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-black text-lg"
                    aria-label="Clear search"
                >
                    &times;
                </button>
                @endif
                <button type="submit" class="bg-black text-white px-4 py-2 rounded hover:bg-gray-800">Cari</button>
            </div>
            <div class="w-48">
                ...
                <tbody class="text-gray-700">
                    ...
                    <tr class="hover:bg-gray-100 border-b border-gray-300">
                        ...
                        <td class="px-5 py-3 border-r align-top whitespace-nowrap">
                            @foreach ($pesanan->details as $detail)
                            <div class="mb-2">
                                {{ (float) $detail->panjang ?? 0 }} x
                                {{ (float) $detail->lebar ?? 0 }} x
                                {{ (float) $detail->tinggi ?? 0 }}
                            </div>
                            @endforeach
                        </td>
                        <td class="px-5 py-3 border-r align-top whitespace-nowrap">
