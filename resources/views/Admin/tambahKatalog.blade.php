        ...
        <form action="{{ route('admin.katalog.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4" required
                    class="w-full border px-4 py-2 focus:outline-none focus:ring focus:border-black">{{ old('deskripsi') }}</textarea>
            </div>
            ...
            <div>
                <label for="gambar" class="block font-semibold mb-1">Gambar Produk (bisa pilih lebih dari satu)</label>
                <input type="file" name="gambar[]" id="gambar" multiple
                    class="w-full border px-4 py-2 focus:outline-none focus:ring focus:border-black">
            </div>

            <div class="text-right">
                <button type="submit" class="bg-black text-white px-6 py-2 hover:bg-gray-800">
                    Simpan Produk
                </button>
            </div>
        </form>
