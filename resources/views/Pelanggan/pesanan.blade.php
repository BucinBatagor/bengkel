...
@section('content')
<section class="bg-gray-200 py-12 min-h-screen"
    x-data="pesananApp()">
    ...
    <div class="max-w-screen-xl mx-auto px-4">
        <div class="bg-white rounded-lg shadow p-6 min-h-[550px] px-4 sm:px-6 lg:px-8">
            ...
            {{-- Flash banner (muncul di bawah judul) --}}
            @if (session('success'))
              ...
              <div class="space-y-8">
                @foreach($pemesanan as $pesanan)
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border rounded-lg p-6 bg-white shadow">
                    <div class="md:col-span-3 flex justify-between items-center mb-4">
                    ...
                    <div class="flex flex-col justify-between items-end">
                      ...
                      <div class="mt-4 w-full">
                        ...
                        @elseif($pesanan->status === 'belum_bayar')
                          <button @click="pay('{{ $pesanan->id }}')" class="w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded">
                            Bayar Sekarang
                          </button>
                        ...
                        @elseif($pesanan->status === 'di_proses')
                          <form action="{{ route('pesanan.ajukan_refund', $pesanan->id) }}" method="POST"
                            ...
                            @csrf
                            <button type="submit" class="w-full bg-gray-700 hover:bg-gray-800 text-white py-2 rounded">
                              Ajukan Refund
                            </button>
                          </form>

                        @elseif($pesanan->status === 'pengembalian_dana')
                          {{-- Tidak ada tombol batalkan refund --}}
                        @endif
                      </div>
                    </div>
                  </div>
    ...
    {{-- Modal serbaguna (hanya untuk konfirmasi/alert runtime, tidak dipicu oleh flash) --}}
    <div x-show="popup.show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="popup.show = false">
      ...
      <div class="relative bg-white rounded-xl shadow-lg w-full max-w-md p-6" x-transition.scale.origin.center
           role="dialog" aria-modal="true" :aria-label="popup.title || 'Dialog'">
        ...
        <div class="flex justify-end gap-2">
          <template x-if="popup.type === 'confirm'">
            <button type="button" class="px-4 py-2 border rounded hover:bg-gray-100"
                    @click="popup.onCancel ? popup.onCancel() : (popup.show = false)">Batal</button>
          </template>
    ...
    {{-- Pagination … (biarkan seperti sebelumnya) --}}
    <div class="flex justify-center mt-6">
      {{-- ... pagination code tetap ... --}}
    </div>
</section>
...
@endphp
<script src="https://app{{ $isProd ? '' : '.sandbox' }}.midtrans.com/snap/snap.js"
    ...
    async pay(id) {
      try {
        ...
        window.snap.pay(snap_token, {
          onSuccess: () => { /* tampilkan banner via flash setelah reload */ location.reload(); },
          onPending: () => { /* opsional: tampilkan alert runtime */ },
