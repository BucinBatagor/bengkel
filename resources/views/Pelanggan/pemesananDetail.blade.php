{{-- resources/views/Pelanggan/pemesananDetail.blade.php --}}
      ...
      <template x-if="allIds.length > 0">
        <div>
          <div class="mb-4 flex items-center">
            <input type="checkbox" id="selectAll" @click="toggleSelectAll()" :checked="isAllSelected()"
                   class="w-4 h-4 mr-2">
            ...
            <div class="space-y-6">
              <template x-for="id in allIds" :key="id">
                <div class="bg-white rounded-lg shadow p-4 relative">
                  <button type="button" @click="confirmDelete(id)"
                          class="absolute top-2 right-2 text-red-600 hover:text-red-800 text-lg font-bold">✕</button>

                  <div class="flex items-center gap-4">
                    <input type="checkbox" :value="id" :checked="selected.includes(id)"
    ...
    openConfirm() {
      if (!this.selected.length) {
        this.popup = {
          ...
          message: 'Pilih setidaknya satu produk sebelum memesan.',
          type: 'alert'
        };
        return;
      }
