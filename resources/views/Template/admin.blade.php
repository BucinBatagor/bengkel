...
<head>
  ...
  <style>
    /* sembunyikan tombol autofill/clear bawaan browser */
        ...
        <nav class="text-sm mt-2 space-y-1">
          <a href="{{ route('admin.pelanggan.index') }}"
             class="block w-full px-6 py-3 flex items-center gap-3 transition
                    {{ request()->routeIs('admin.pelanggan.*') ? 'bg-gray-600 text-white' : 'hover:bg-gray-800 text-white' }}">
        ...
        <nav class="text-sm space-y-1">
          <div x-data="{ open: false }" class="relative">
            <button @click="open = !open"
                    class="block w-full px-6 py-3 flex items-center justify-between gap-3
                           text-white hover:bg-gray-800 rounded transition">
            ...
            <div x-show="open" @click.away="open = false" x-transition
              ...
              <a href="{{ route('admin.profil') }}"
                 class="block w-full px-6 py-2 hover:bg-gray-800 transition
                        {{ request()->routeIs('admin.profil') ? 'bg-gray-600' : '' }}">
                Ubah Profil
              </a>
              <a href="{{ route('admin.profil.password') }}"
                 class="block w-full px-6 py-2 hover:bg-gray-800 transition
                        {{ request()->routeIs('admin.profil.password') ? 'bg-gray-600' : '' }}">
                Ubah Password
              </a>
            </div>
          </div>
          <form method="POST" action="{{ route('admin.logout') }}">
            @csrf
            <button type="submit"
                    class="block w-full px-6 py-3 flex items-center gap-3
                           text-red-400 hover:text-white hover:bg-red-800 rounded transition">
